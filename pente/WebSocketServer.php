...
require_once("PenteUser.php");

abstract class WebSocketServer{
    protected $userClass = 'PenteUser';
    ...
    function __construct($addr, $port, $bufferLength = 2048){
        ...
        $this->stdout("Server started. Listening on: ws://$addr:$port.");
    }
                ...
                if($socket == $this->master){
                    ...
                    if($numBytes === false){
                        ...
                        try{
                            ...
                            $this->stdout("Could not send error message to client.");
                        }
                        $this->stderr("A fatal server crash has been avoided. Potential crash caused by " . $user->name . ". This user is being disconnected.");
    ...
    public function stdout($message){
        if($this->interactive){
            $data = "[INFO] [" . date("F j, Y", (time())) . " " . date("H:i:s", (time())) . "] " . $message . "\n";
            echo($data);
            $file = '/code/logs/pente.log';
    ...
    public function stderr($message){
        if($this->interactive){
            $data = "[ERROR] [" . date("F j, Y", (time())) . " " . date("H:i:s", (time())) . "] " . $message . "\n";
            echo($data);
            $file = '/code/logs/pente.log';
            file_put_contents($file, $data, FILE_APPEND | LOCK_EX);
        }
    ...
    public static function matchLog($message){
        $data = "[MATCH] [" . date("F j, Y", (time())) . " " . date("H:i:s", (time())) . "] " . $message . "\n";
        echo($data);
        $file = '/code/logs/pente.log';
        file_put_contents($file, $data, FILE_APPEND | LOCK_EX);
    }
